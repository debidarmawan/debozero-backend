...
const getEntryLists = `-- name: GetEntryLists :many
SELECT id, account_id, amount, created_at FROM entries ORDER BY id LIMIT $1 OFFSET $2
`

type GetEntryListsParams struct {
	Limit  int32 `json:"limit"`
	Offset int32 `json:"offset"`
}

func (q *Queries) GetEntryLists(ctx context.Context, arg GetEntryListsParams) ([]Entry, error) {
	rows, err := q.db.QueryContext(ctx, getEntryLists, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()

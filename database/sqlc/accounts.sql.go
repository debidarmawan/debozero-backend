...
const getAccountLists = `-- name: GetAccountLists :many
SELECT id, user_id, balance, currency, created_at FROM accounts ORDER BY id LIMIT $1 OFFSET $2
`

type GetAccountListsParams struct {
	Limit  int32 `json:"limit"`
	Offset int32 `json:"offset"`
}

func (q *Queries) GetAccountLists(ctx context.Context, arg GetAccountListsParams) ([]Account, error) {
	rows, err := q.db.QueryContext(ctx, getAccountLists, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
